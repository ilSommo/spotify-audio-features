...
def graph(sizes, exact_results, likelihood_weighted_results, rejection_results, variable, evidence):
	...
	for state in exact_results.keys():
		...
		plt.ylim(0, 1)
		plt.title(state)
		likelihood_weighted_plot, _ = plt.semilogx(sizes, likelihood_weighted_results[state], label = "Likelihood weighted sampling")
		rejection_plot, _ = plt.semilogx(sizes, rejection_results[state], label = "Rejection sampling")
		exact_plot, _ = plt.semilogx(sizes, exact_results[state] * np.ones(len(sizes)), label = "Exact inference")
		plt.legend(handles = [exact_plot, likelihood_weighted_plot, rejection_plot])
	plt.show()


def graph_diff(sizes, exact_results, likelihood_weighted_results, rejection_results, variable, evidence):
	...
	for state in exact_results.keys():
		...
		plt.ylim(-0.5, 0.5)
		plt.title(state)
		likelihood_weighted_plot, _ = plt.semilogx(sizes, likelihood_weighted_results[state] - exact_results[state], label = "Likelihood weighted sampling")
		rejection_plot, _ = plt.semilogx(sizes, rejection_results[state] - exact_results[state], label = "Rejection sampling")
		exact_plot, _ = plt.semilogx(sizes, np.zeros(len(sizes)), label = "Exact inference")
		plt.legend(handles = [exact_plot, likelihood_weighted_plot, rejection_plot])
	plt.show()